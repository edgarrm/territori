...
- **Fase**: 05/06 (SDD + Implementation), Sprint 5 cerrado.
...
- **Pendiente inmediato (Sprint 6)**: Interacciones (timeline) + agenda de seguimientos + export CSV + derechos ARCO (cancelación/`solicitudes_arco`) + modo de captura `evento` (tabla `eventos` y el FK pendiente en `electores`). `ultimo_registro` del resumen del mapa (hoy `null`) puede poblarse aquí.
